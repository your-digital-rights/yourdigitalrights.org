...
export default {
  ...
  body(requestItem, complaintText, status) {
    ...
    bodyParts.push(`On ${new Intl.DateTimeFormat('en', { dateStyle: 'full'}).format(new Date(requestItem.requestCreatedAt.S))} I sent ${requestItem.companyName.S} a Data ${capitalize(requestType.name)} Request pursuant to article ${requestType.article} of the ${regulation.longName} (${regulation.displayName}).`);

    if (requestItem.reminderEmailsSentDate) {
      bodyParts.push(`I have also sent the organization a reminder email on ${dateFormatter(new Date(requestItem.reminderEmailsSentDate.S))}.`);
    }
    ...
    bodyParts.push(`I would like to file a formal complaint because ${reason}, and because it has been ${daysSinceRequest} days since the request was sent. Please keep me updated as to the status of your investigation. Please let me know if you would like me to provide any additional information. My preferred method of communication is email.`);
